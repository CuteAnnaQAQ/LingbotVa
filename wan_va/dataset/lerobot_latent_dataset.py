...
def construct_lerobot_multi_processor(config, 
    ...
    # `os.walk` 返回当前平台的路径分隔符；通过 pathlib 推导数据集根目录，
    # 可以同时兼容 Linux 和 Windows。
    repo_list = [str(Path(v).parent.parent) for v in repo_list]
    if not repo_list:
...
class LatentLeRobotDataset(LeRobotDataset):
    def __init__(
        ...
        # `construct_lerobot_multi_processor` 传入的是已发现的本地数据集根目录，
        # 而不是 Hub 仓库 ID；直接使用该路径也能稳定定位同级的 `latents/`。
        self.repo_id = str(repo_id)
        self.root = Path(repo_id).resolve()
        ...
        if not info_path.is_file():
            raise FileNotFoundError(
                f'LeRobot metadata not found: {info_path}. Download the full '
                'dataset before constructing the LingBot latent loader.')
        ...
        missing_episode_files = [
            self.root / fpath for fpath in self.get_episodes_file_paths()
            if not (self.root / fpath).is_file()
        ]
        if missing_episode_files:
            preview = ', '.join(str(v) for v in missing_episode_files[:3])
            suffix = '' if len(missing_episode_files) <= 3 else ' ...'
            raise FileNotFoundError(
                f'Incomplete local LeRobot dataset: missing '
                f'{len(missing_episode_files)} episode file(s): '
                f'{preview}{suffix}')
        ...
        self.action_representation = getattr(
            config, 'action_representation', 'dataset')
        if (getattr(config, 'require_action_stats', False)
                and not getattr(config, 'action_stats_path', '')):
            raise ValueError(
                'RoboMME training requires exact action statistics. Set '
                'LINGBOT_ROBOMME_ACTION_STATS to the JSON generated by '
                'wan_va.utils.compute_robomme_action_stats.')
        self.q01 = np.array(config.norm_stat['q01'], dtype='float')[None]
        self.q99 = np.array(config.norm_stat['q99'], dtype='float')[None]
        hf_columns = [self.dataset_action_key]
        if self.action_representation == 'joint_delta':
            hf_columns.append(self.dataset_state_key)
    ...
    def parse_meta(self):
        ...
        for key, value in self.meta.episodes.items():
            ...
            if not action_config:
                raise ValueError(
                    f'Episode {episode_index} has no action_config. RoboMME '
                    'raw episodes.jsonl is LeRobot-compatible but must be '
                    'augmented before LingBot latent extraction/training.')
        ...
        if not out:
            raise FileNotFoundError(
                f'No complete LingBot latent segments found under '
                f'{self.latent_path} for camera keys {self.used_video_keys}.')
        self.new_metas = out
    ...
    def _get_range_hf_data(self, start_frame, end_frame):
        ...
        output = {'action': batch[self.dataset_action_key]}
        if self.action_representation == 'joint_delta':
            output['state'] = batch[self.dataset_state_key]
        return output
    ...
    def _action_post_process(self, local_start_frame, local_end_frame,
                             latent_frame_ids, action, state=None):
        action = np.asarray(action, dtype=np.float32)
        if self.action_representation == 'joint_delta':
            if state is None:
                raise ValueError('joint_delta action processing requires state')
            state = np.asarray(state, dtype=np.float32)
            if action.shape != state.shape or action.shape[-1] != 8:
                raise ValueError(
                    f'Expected matching RoboMME action/state shapes (*, 8), got '
                    f'{action.shape} and {state.shape}')
            action = action.copy()
            action[:, :7] -= state[:, :7]
        elif self.action_representation not in ('dataset', 'absolute_joint'):
            raise ValueError(
                f'Unsupported action representation '
        ...
        if self.config.env_type == 'robotwin_tshape':  # TODO：目前仅 RobotWin 支持相对位姿，后续扩展到其他数据集。
